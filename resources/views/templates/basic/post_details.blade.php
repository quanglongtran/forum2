...
@php
    ...
    $parent_id = 0;
@endphp
<div style="display: none" id="user-{{$rand}}">{{Auth::user()}}</div>
  ...
  @foreach($comments as $comment)
    ...
    @endif
      <div class="single-comment">
        ...
        <div class="mt-2 w-100" style="position: relative">
          <span style="position: relative">
            {!! nl2br($comment->comment) !!}
            <div style="position: absolute;left: 88%; font-size: 11px;top:65%">
                <div class="" onclick="replyModal(this)" comment-id="{{$comment->id}}" parent-id="{{$parent_id++}}"
                    data-bs-toggle="modal" data-bs-target="#reply-modal">Reply</div>
            </div>
          ...
          <div class="reply" style="--level:{{$reply->level}};">
            <span style="position: relative" class="reply-text">
              {{$reply->reply}}
                <div style="position: absolute">
                    <div class="reply-btn" onclick="replyModal(this)" parent-id="{{$reply->id}}" comment-id="{{$comment->id}}" data-bs-toggle="modal" data-bs-target="#reply-modal">Reply</div>
                </div>
            </span>
          </div>
          @endforeach
...
<div class="modal fade" id="reply-modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="text" id="reply-modal-input" autocomplete="off">
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary" onclick="reply($(this),$('#reply-modal-input').val())" id="send-reply-btn">Save changes</button>
      </div>
    </div>
  </div>
</div>
  <script>
    ...
    var user = JSON.parse($('#user-{{$rand}}').text())

    function replyModal(element) {
      let _token = "{{ csrf_token() }}"
      $('#send-reply-btn').attr('comment-id', element.getAttribute('comment-id')).attr('parent-id', element.getAttribute('parent-id'))
        
    }
    function reply(element, reply) {
      if (!"{{Auth::check()}}") {
        return alert('Bạn chưa đăng nhập');
      }
      // return console.log(element, reply)
      $.ajax({
        url: "{{route('forum.reply')}}",
        type: 'POST',
        data: {
          comment_id: element.attr('comment-id'),
          parent_id: element.attr('parent-id'),
          reply: reply,
          user_id: user.id,
        },
        success: response => {
...
<style>
  ...
  #commentArea .reply .reply-text>div {
    left: 88%;
    top: 65%;
  }
  #commentArea .reply .reply-btn {
    font-size: 11px;
    cursor: pointer;
    transition: all 0.2s ease;
  }
  #commentArea .reply .reply-btn:hover {
    color: orange;
  }
  #commentArea .reply .input {
    height: 10px;
    top: calc(10% * var(--level));
  }
</style>
@endpush