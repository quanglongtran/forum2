...
<main>
    ...
    <script>
        const slug = (str, separate = '') => {
            return str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replaceAll(' ', separate).replaceAll('đ', 'd').toLowerCase();
        }
    ...
    <script>
        ...
        const appendRNList = (side = left, item, index, parent) => {
            ...
            $('.rn-list.' + side).append(
                `<div class="post-item post-list">
                    ...
                    <a href="{{asset('forum/post/${postSlug}/${item.id}')}}" class="newlist-item-title">${item.post_title}</a>
                ...
                `
                    <div class="col-md-3 col-sm-6 col-12">
                        <div class="event-item">
                            <div class="box-item box-text-bottom post-item">
                                <div class="img">
                                    <a href="{{asset('forum/post/${postSlug}/${item.id}')}}" class="plain">
                                        <div class="box-image">
                                            <div class="image-cover" style="padding-top: 56.67%;">
                                                <img src="${item.image}">
                                            </div>
                                ...
                                <div class="box-text text-left">
                                    ...
                                    <div class="box-text-inner blog-post-inner">
                                        <a href="{{asset('forum/post/${postSlug}/${item.id}')}}" class="h5 post-title is-large ">${item.post_title}</a>
                                    </div>
        ...
        for (let i of newsAndEvents) {
            if (i.name === 'Tin tức đề xuất') {
                ...
                let postSlug = slug(i.posts[0].post_title, '-');
                $('#newshome-readmore-{{$rand}}').attr('href', `{{asset('forum/category/${catSlug}/${i.id}')}}`)
                $('#rn-thumbnail').attr('src', i.posts[0].image)
                $('#rn-thumbnail').parent().attr('href', `{{asset('forum/post/${postSlug}/${i.posts[0].id}')}}`)
    ...
    <script>
        ...
        const appendDLTopic = (item, index, id) => {
            ...
            let thumbnail = (item.posts?.user?.photo) ? item.posts.user.photo : "http://27.71.234.22/placeholder-image/350x300";
    ...
    <script>
        ...
        for (let i in digital_library) {
            let time = digital_library[i].time;
            
            $('#db__body-item-{{$rand}}').append(
                `<div class="db__body-item">
                    <a href="#" class="statistics" style="display: flex; align-items: center">
                        <span class="icon">
                            <svg width="40" height="40" viewBox="0 0 40 40" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                        ...
                        <span style="-webkit-line-clamp: 2;-webkit-box-orient: vertical;overflow: hidden;text-overflow: ellipsis;flex:1;display: -webkit-box;" class="edit">${digital_library[i].post_title}</span></a>
                    <span class="created">11/08/2021</span>
                    <span class="title">
                        <span>
